...
<script runat="server">
            ...
            If Util.IsInternalUser2() AndAlso (Session("account_status") <> "FC" Or Util.IsFranchiser(Session("user_id"), "")) Then
                ...
                If String.IsNullOrEmpty(EQpaymentTerm) Then
                   ...
                   String.Format("select top 1 CREDIT_TERM from SAP_DIMCOMPANY where company_id='{0}' and org_id='{1}'", _
                                 Session("company_id"), Session("org_id")))
    ...
    Sub initInterface()
        ...
        If mycart.isQuote2Order(CartId, quoteId) Then
            ...
            'Dim WS As New quote.quote-Exit : WS.Timeout = -1
            ...
            Dim QuotePartners As List(Of EQPARTNER) = eQuotationUtil.GetEQPartnerByQuoteid(quoteId)
            Dim QuoteNotes As List(Of QuotationNote) = eQuotationUtil.GetQuotationNoteByQuoteid(quoteId)
            If MyQuoteMaster IsNot Nothing Then
                If QuotePartners IsNot Nothing AndAlso QuotePartners.Count > 0 Then
                    For Each partner As EQPARTNER In QuotePartners
                        With partner
                    ...
                    End With
                End If
                If QuoteNotes IsNot Nothing AndAlso QuoteNotes.Count > 0 Then
                    For Each dr As QuotationNote In QuoteNotes
        ...
        Me.txtIncoterm.Text = incotermText : Me.txtSalesNote.Text += vbLf + MYSAPBIZ.getSalesNotebyCustomer(Session("Company_id")).Trim()
    ...
    Sub DBfromCart2Order(ByVal Cart_ID As String)
        ...
        End If
        Dim CreditCardNumber As String = txtCreditCardNumber.Text.Replace("'", "''")
        Dim VerifyNumber As String = txtCCardVerifyValue.Text.Replace("'", "''")
        ...
        myOrderMaster.Add(ORDER_ID, ORDER_NO, ORDER_TYPE, PO_NO, PO_DATE, SOLDTO_ID, SHIPTO_ID, CURRENCY, MREQDATE, txtBillTo.Text, "", ORDER_DATE, "", ATTENTION, PARTIALFLAG, _
                          ...
                          CreditCardExpireDate, VerifyNumber, dlPayterm.SelectedValue, dlCCardType.SelectedValue, txtCCardHolder.Text, txtBillingInstructionInfo.Text, ddlSE.SelectedValue, _
    ...
    Protected Sub SyncCustomerID(ByVal OrderID As String)
        ...
        For Each op As MyOrderDS.ORDER_PARTNERSRow In OPdt
            If Not String.IsNullOrEmpty(op.ERPID) Then
        ...
        For Each OrderAddress As OrderAddress In OrderAddressS
            With OrderAddress
                If Not String.IsNullOrEmpty(.ERPID.Trim) Then
                    A.Insert(CartId, "", .ERPID.ToUpper.Trim, .Name.Trim, "", .Type.Trim, .Attention.Trim, .Tel.Trim, "", .Zipcode.Trim, .Country.Trim, .City.Trim, .Street.Trim, .State.Trim, "", .Street2.Trim, .taxJuri.Trim)
                End If
            End With
        Next
