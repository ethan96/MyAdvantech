    ...
    */
	...
	function closeCalendar() {
        ...
        //default_due_date
        hideCalendar();
        new_due_date = constructDate(dateSelected,monthSelected,yearSelected)
        if (new_due_date<default_due_date)
            alert("Please keep the due date later than given default due date "+default_due_date + "!" )
        else
   ...
   function WeekNbr(n) {
	...
	function popUpCalendar(ctl,	ctl2, format,val) {
		var	leftpos=0
		var	toppos=0

        default_due_date = val
        //alert (default_due_date)
		if (bPageLoaded)
		{
			...
			{
				hideCalendar()
				if (ctlNow!=ctl) {popUpCalendar(ctl, ctl2, format,val)}
			}
			ctlNow = ctl
		}
	}
