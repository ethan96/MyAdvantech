<%@ Page Language="VB" EnableEventValidation="false"  ValidateRequest="false" MasterPageFile="~/Includes/MyMaster.master"
    Title="CBOM Editor" %>
...
<script runat="server">    
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        'If IsNothing(Session("user_id")) Or IsNothing(Session("org")) Then
        If IsNothing(Session("user_id")) Or IsNothing(Session("org_id")) Then
            Response.Write("~/login.aspx")
        End If
        'apply to production
        'If Util.IsTesting() Then
        If Not IsNothing(Request("UID")) AndAlso Request("UID") <> "" Then
            Response.Redirect("~/WebCBOMEditor/CBOM_Editor_eq.aspx?UID=" & Request("UID"))
        End If
        If Not IsNothing(Request("BTOItem")) AndAlso Request("BTOItem") <> "" Then
            Response.Redirect("~/WebCBOMEditor/CBOM_Editor_eq.aspx?BTOItem=" & Request("BTOItem"))
        End If
           
        'End If
        ' Response.Write(Session("user_id") & Session("org"))
        If Not Page.IsPostBack Then
            OrderUtilities.SetSessionOrgForCbomEditor(Session("user_id"))
        End If
        If Not Page.IsPostBack Then
            If Not IsNothing(Request("UID")) AndAlso Request("UID") <> "" Then
                Dim ITEM As DataTable = dbUtil.dbGetDataTable(CBOMSetting.DBConn, "SELECT * FROM CBOM_CATALOG_CATEGORY WHERE UID='" & Request("UID") & "'")
            ...
            If Left(Session("org_id").ToString.ToUpper, 2) = "EU" Then
                Me.rbtnIsByReg.SelectedValue = 1
            Else
                Me.rbtnIsByReg.SelectedValue = 0
            End If
            buildBom()
        End If
    End Sub
    Sub buildBom()
        Dim dt As DataTable = Util.GetQBOMSql(Me.BTOItemTb.Text, Left(Session("org_id").ToString.ToUpper, 2), IIf(Me.rbtnIsByReg.SelectedValue = 0, False, True))
    ...
    Sub OnSelectedItemChanged(ByVal s As Object, ByVal e As System.EventArgs) Handles tv1.SelectedNodeChanged
        ...
        Dim itemDt As DataTable = dbUtil.dbGetDataTable(CBOMSetting.DBConn, tempSql)
    ...
    Sub AppendSubNodes(ByRef Sel_Node As AdxTreeNode)
        ...
        Dim dt As DataTable = Util.GetQBOMSql(Sel_Node.Value, Left(Session("org_id").ToString.ToUpper, 2), IIf(Me.rbtnIsByReg.SelectedValue = 0, False, True))
        If dt Is Nothing Then Exit Sub
        Dim catDt As New DataTable, compDt As New DataTable
        catDt = dt.Copy()
        compDt = dt.Copy()
        ...
        If catDt.Rows.Count = 0 Then
            ...
            For i As Integer = 0 To compDt.Rows.Count - 1
                ...
                Dim compN As New AdxTreeNode
                'If compDt.Rows(i).Item("status").ToString.ToUpper <> "A" And _
                '    compDt.Rows(i).Item("status").ToString.ToUpper <> "N" And _
                '    compDt.Rows(i).Item("status").ToString.ToUpper <> "H" Then
                If compDt.Rows(i).Item("status").ToString.ToUpper = "O" or compDt.Rows(i).Item("status").ToString.ToUpper = "I" Then
                    compN.Text = compDt.Rows(i).Item("seq_no") & " " & compDt.Rows(i).Item("category_id") & " (phase out)"
                ElseIf compDt.Rows(i).Item("status").ToString.ToUpper = "H" Then
                    compN.Text = compDt.Rows(i).Item("seq_no") & " " & compDt.Rows(i).Item("category_id") & " (hold shipment)"
                Else
                    compN.Text = compDt.Rows(i).Item("seq_no") & " " & compDt.Rows(i).Item("category_id")
    ...
    Handles CatAdd.Click, CatDel.Click, CatUpdate.Click, CompAdd.Click, CompDel.Click, CompUpdate.Click
        ...
        Dim sCatId As String = Me.CatNameTb.Text.Trim().Replace("'", "''")
        Dim sCatDesc As String = Me.CatDesc.Text.Trim().Replace("'", "''")
        Dim sCatRef As String = Me.txtRefCategory.Text.Trim().Replace("'", "''")
        ...
        Dim sCompId As String = Me.CompNameTb.Text.Trim().Replace("'", "''")
        Dim sCompDesc As String = Me.CompDesc.Text.Trim().Replace("'", "''")
        Dim sCompRef As String = Me.txtRefCategory1.Text.Trim().Replace("'", "''")
        ...
        Select Case (b.ID.ToLower())
            ...
            Case "catadd"
                ...
                Dim n As New AdxTreeNode
                n.Text = Me.CatSeqNo.Text.Replace("'", "''") & " " & Me.CatNameTb.Text.Trim().Replace("'", "''") : n.Value = Me.CatNameTb.Text.Trim().Replace("'", "''")
                ...
                If IsNumeric(Me.CatSeqNo.Text.Replace("'", "''")) Then n.xSeqNo = CDbl(Me.CatSeqNo.Text.Replace("'", "''"))
                ...
                'Frank 2012/06/04: Stop using Session("org") and replacing by Left(Session("org_id"), 2)
                'dt = dbUtil.dbGetDataTable(CBOMSetting.DBConn, _
                ...
                " CONFIGURATION_RULE,org,uid) " & _
                " VALUES ('" & sCatId & "', '" & sCatId & "', 'Category', " & _
                " N'" & Me.tv1.SelectedNode.Parent.Value & "', '" & sCatDesc & "', '" & _
                Me.tv1.Nodes(0).Value & "', '" & Session("user_id") & "', " & _
                sCatSeqNo & ", '" & sCatRequired & "','" & Left(Session("org_id").ToString.ToUpper, 2) & "',newid()) "

                Me.DebugSql.Text = addSql
                dbUtil.dbGetDataTable(CBOMSetting.DBConn, addSql)
                strAddItem = sCatId
                'Frank 2012/06/04: Stop using Session("org") and replacing by Left(Session("org_id"), 2)
                ...
                dbUtil.dbGetDataTable(CBOMSetting.DBConn, UpdateSeqSql)
            ...
            Case "catdel"
                ...
                'Frank 2012/06/04: Stop using Session("org") and replacing by Left(Session("org_id"), 2)
                'Dim pCountDt As DataTable = dbUtil.dbGetDataTable(CBOMSetting.DBConn, _
                ...
                End If
                Me.DebugSql.Text = Me.tv1.SelectedValue
                'Response.Write(pCountDt) : Response.End()
                dbUtil.dbGetDataTable(CBOMSetting.DBConn, delSql)
                strAddItem = sCatId
            Case "catupdate"
                If Me.MultiView1.ActiveViewIndex <> 0 Then Exit Sub
                If Me.CatNameTb.Text.Trim() = "" Then Exit Sub
                ...
                strAddItem = Me.CatNameTb.Text.Trim()
                ...
                dbUtil.dbGetDataTable(CBOMSetting.DBConn, updateSql)
                If Me.tv1.SelectedNode.Depth > 0 Then
            ...
            Case "compadd"
                ...
                Dim pcid As String = Me.tv1.SelectedNode.Value
                ...
                '" VALUES ('" & sCompId.Replace("'", "''") & "', '" & sCompId.Replace("'", "''") & "', 'Component', " & _
                '" '" & pcid.Replace("'", "''") & "', '" & sCompDesc.Replace("'", "''") & "', '" & _
                'Me.tv1.Nodes(0).Value.Replace("'", "''") & "', '" & Session("user_id") & "', " & _
                'sCompSeqNo & ", '" & sCompDefault.Replace("'", "''") & "', '" & sCompNotExpand.Replace("'", "''") & "', '" & sCompShowHide & "','" & Session("org").ToString.ToUpper & "',NEWID()) "
                ...
                " CONFIGURATION_RULE, NOT_EXPAND_CATEGORY, SHOW_HIDE,ORG,UID) " & _
                " VALUES ('" & sCompId.Replace("'", "''") & "', '" & sCompId.Replace("'", "''") & "', 'Component', " & _
                " N'" & pcid.Replace("'", "''") & "', '" & sCompDesc.Replace("'", "''") & "', '" & _
                Me.tv1.Nodes(0).Value.Replace("'", "''") & "', '" & Session("user_id") & "', " & _
                sCompSeqNo & ", '" & sCompDefault.Replace("'", "''") & "', '" & sCompNotExpand.Replace("'", "''") & "', '" & sCompShowHide & "','" & Left(Session("org_id").ToString.ToUpper, 2) & "',NEWID()) "
            ...
            Case "compdel"
                ...
                dbUtil.dbGetDataTable(CBOMSetting.DBConn, delSql)
                If tv1.SelectedNode IsNot Nothing AndAlso tv1.SelectedNode.Parent IsNot Nothing Then
                    strAddItem = Me.tv1.SelectedNode.Parent.Value
                End If
                'Dim log_str As String = String.Format("insert into cbom_maintain_log values(newid(),'{0}','{1}','{3}',getdate(),'{2}',0)", Session("user_id"), Me.tv1.SelectedValue, Session("org"), Me.tv1.SelectedNode.Parent.Value)
                'dbUtil.dbExecuteNoQuery(CBOMSetting.DBConn, log_str) 
            Case "compupdate"
                If Me.MultiView1.ActiveViewIndex <> 1 Then Exit Sub
                If Me.CompNameTb.Text.Trim() = "" Then Exit Sub
                ...
                If Me.tv1.SelectedNode.Depth > 0 Then
                    updateSql &= " and parent_category_id='" & Me.tv1.SelectedNode.Parent.Value & "'"
                End If
                strAddItem = Me.CompNameTb.Text.Trim()
                Me.DebugSql.Text = updateSql
                dbUtil.dbGetDataTable(CBOMSetting.DBConn, updateSql)
        ...
        End Select
        'Frank 2012/06/04: Stop using Session("org") and replacing by Left(Session("org_id"), 2)
        'Dim log_str As String = String.Format( _
        '    "insert into cbom_maintain_log values(newid(),'{0}','{1}','{2}',getdate(),'{3}','{4}')", _
        '    Session("user_id"), Replace(Me.BTOItemTb.Text, "'", "''"), Replace(strAddItem, "'", "''"), Session("org"), Replace(b.Text, "'", "''"))
        Dim log_str As String = String.Format( _
            "insert into cbom_maintain_log values(newid(),'{0}','{1}','{2}',getdate(),'{3}','{4}')", _
            Session("user_id"), Replace(Me.BTOItemTb.Text, "'", "''"), Replace(strAddItem, "'", "''"), Left(Session("org_id").ToString.ToUpper, 2), Replace(b.Text, "'", "''"))

        dbUtil.dbExecuteNoQuery(CBOMSetting.DBConn, log_str)
    End Sub
    
    Protected Sub rbtnIsByReg_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        buildBom()
    End Sub
</script>
<asp:Content runat="server" ID="_main" ContentPlaceHolderID="_main" >
    <script src="../Includes/jquery-1.11.1.min.js" type="text/javascript"></script>
    <script src="../Includes/jquery_easydrag.js" type="text/javascript"></script>
    <script>
    ...
    <script language="javascript" for="document" event="onkeydown">
     $(document).ready(function(){  
        ...
        event.returnValue = false; 
             var aa = document.getElementById('ctl00__main_hd').value;
             if ( aa=="1")
             {     
              PickCompWin();
             }
    ...
    <table><tr><td>
    Show Category By Region :</td><td> <asp:RadioButtonList runat="server" ID="rbtnIsByReg" OnSelectedIndexChanged="rbtnIsByReg_SelectedIndexChanged" RepeatDirection="Horizontal" AutoPostBack="true">
    <asp:ListItem Value="1" Selected ="True">By Region</asp:ListItem>
    ...
    <a href='http://<%=Request.ServerVariables("HTTP_HOST") %>/Order/Configurator.aspx?BTOItem=<%=me.BTOItemTb.text %>&Qty=1'
    ...
    <div id="divPanel" style="background: white; border: 1px solid #000000; position: absolute;
        left: 500px; top: 50px;">
                        ...
                        <asp:View ID="CatView" runat="server">
                            <asp:Table ID="CatEditTable" runat="server">
                                ...
                                <asp:TableRow ID="TableRowCopy" runat="server">
                                    <asp:TableCell ID="TableCellCopy" Text="" runat="server">
                                        <asp:CheckBox ID="chkCat" runat="server"></asp:CheckBox></asp:TableCell>
                                    <asp:TableCell ID="TableCellCopy1" runat="server">
                                        Copy From:
                                        <asp:TextBox ID="txtRefCategory" runat="server" Width="250px"></asp:TextBox>
                                    </asp:TableCell>
                                </asp:TableRow>
                            ...
                            <asp:Table ID="CompEditTable" runat="server">
                                ...
                                <asp:TableRow ID="TableRow11" runat="server">
                                    ...
                                    <asp:TableCell ID="TableCell17" runat="server">
                                        ...
                                        <input type="button" id="btnSearch" value="Pick Existed Component" onclick="PickCompWin();" />
                                    </asp:TableCell>
                                </asp:TableRow>
                                <asp:TableRow ID="TableRowCopy2" runat="server">
                                    <asp:TableCell ID="TableCellCopy2" Text="" runat="server">
                                        <asp:CheckBox ID="chkComp" runat="server"></asp:CheckBox></asp:TableCell>
                                    <asp:TableCell ID="TableCellCopy21" runat="server">
                                        Copy From:
                                        <asp:TextBox ID="txtRefCategory1" runat="server" Width="250px"></asp:TextBox>
                                    </asp:TableCell>
                                </asp:TableRow>
...
function PickCatWin()
	{
	    ...
	    var part_no = aa.value
	    //alert (part_no)http://<%=Request.servervariables("HTTP_HOST") %>
	    var Url = "../Order/PickCategory.aspx?Type=QueryPrice&Element=CatNameTb&Element2=CatDesc"
	    Url=Url + "&PartNo=" + part_no
		window.open(Url, "pop","height=570,width=520,scrollbars=yes");
       // return false;
	}	
