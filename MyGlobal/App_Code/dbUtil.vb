...
Public Class dbUtil
    ...
    ByVal strSqlCmd As String) As Integer
        ...
        Try
            ...
            retInt = dbCmd.ExecuteNonQuery()
            'tran.Commit()
            'ICC 2017/6/23 Prevent SQL deadlock, add SqlException section to capture SQL deadlock exception and try to re-excute this SQL after sleeping 1 second.
    ...
    Public Shared Function dbExecuteNoQueryShareConn( _
    ByVal Connection As SqlConnection, _
    ByVal strSqlCmd As String) As Integer
        Dim g_adoConn As SqlConnection = Connection
        Dim dbCmd As SqlClient.SqlCommand = g_adoConn.CreateCommand()
        dbCmd.Connection = g_adoConn : dbCmd.CommandText = strSqlCmd
        Dim retInt As Integer = -1
        For i As Integer = 0 To 3
            Try
                If Not g_adoConn.State = ConnectionState.Open Then g_adoConn.Open()
                Exit For
            Catch ex As SqlException
                If i = 3 Then Throw ex
                Threading.Thread.Sleep(100)
