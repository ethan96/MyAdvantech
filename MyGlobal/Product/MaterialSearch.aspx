...
<script runat="server">
    ...
    Function GetMarketingTypes() As List(Of String)
        Dim arrTypes As New List(Of String)
        'cblLitSearch選取項目數量
        Dim SelectedTypes As Integer = Util.GetCheckedCountFromCheckBoxList(cblLitSearch)
        hf_LitSearch.Value = CStr(SelectedTypes)
                ...
                If li.Selected Then
                    ...
                    If tmpType = "photo" Then
                        ...
                        For Each ctr As DataRow In certTypeDt.Rows
                            arrTypes.Add("'" + ctr.Item("LIT_TYPE").ToString.ToLower + "'")
                        Next
                    Else
    ...
    Function GetCMSDomainByOrg() As List(Of String)
        Dim dt As DataTable = dbUtil.dbGetDataTable("MY", String.Format("select a.DOMAIN from CurationPool.dbo.CMS_DOMAIN_ORG a inner join (select case z.SIEBEL_RBU when 'ACL' then 'ACL' when 'ARU' then 'ARU' when 'AID' then 'AID' when 'AMY' then 'AMY' when 'ASG' then 'ASG' when 'ATH' then 'ATH' when 'APH' then 'APH' when 'AMX' then 'AMX' else z.FLASHLEADS_RBU end as FLASHLEADS_RBU, z.SIEBEL_RBU from CurationPool.dbo.LEADSFLASHRBU_SIEBELRBU z) b on a.ORG=b.FLASHLEADS_RBU where b.SIEBEL_RBU in (select RBU from SIEBEL_ACCOUNT where ERP_ID ='{0}') ", Session("COMPANY_ID")))
        Dim Domains As New List(Of String)
        For Each row As DataRow In dt.Rows
            Domains.Add(row.Item("DOMAIN").ToString)
        Next
        Return Domains
    End Function

    Function GetCMSData() As DataTable
        Dim cms_types As New List(Of String) From {"video", "case study", "news", "white papers", "ecatalog", "edm / enewsletter", "poster", "presentation slide", "corporate identity system (cis)"}
        Dim arrTypes As List(Of String) = GetMarketingTypes()
        Dim iclbbu As Integer = Util.GetCheckedCountFromCheckBoxList(cblBU), pickCorp As Boolean = False

        If iclbbu = 1 Then
            For Each item As ListItem In cblBU.Items
                If item.Selected AndAlso item.Value = "CORP" Then
                    pickCorp = True
                End If
            Next
        End If
        ...
        With sb
            .AppendFormat(" SELECT DISTINCT top 100 '' AS LIT_ID, M.Title AS LIT_NAME, M.ModifyDate AS LAST_UPD, isnull(M.Abstract,'') AS DESC_TEXT, D.DisplayName AS LIT_TYPE, ")
            .AppendFormat(" '' AS FILE_EXT, cast(0 as int) as FILE_SIZE, M.URL AS FTP_URL, (SELECT top 1 FilePath FROM ExtensionFile WHERE (CmsID = M.CmsID) AND (Did IN (SELECT Did FROM Dictionary WHERE (DisplayName = 'Featured Image')))) AS RECORD_IMG, M.CmsID AS RECORD_ID,  ")
            .AppendFormat(" '' AS SR_TYPE, M.URL AS HYPER_LINK ")
            .AppendFormat(" From Master AS M (nolock) ")
            .AppendFormat(" LEFT OUTER JOIN Dictionary AS D (nolock) ON M.Type = D.Did  ")
            .AppendFormat(" LEFT OUTER JOIN ExtensionEvents AS EE (nolock) ON M.CmsID = EE.CmsID ")
            .AppendFormat(" LEFT OUTER JOIN (select z.CmsID, cast(z.RelationKey as varchar(36)) as RelationKey, z.RelationType from Relation z (nolock)) R ON M.CmsID=R.CmsID and LEN (R.RelationKey) = 36  ")
            .AppendFormat(" LEFT OUTER JOIN (select CAST(z.Did as varchar(36)) as Did, z.DataType, z.Description, z.DisplayName from Dictionary z (nolock)) DR on R.RelationKey=DR.Did ")
            .AppendFormat(" WHERE (M.Status = 1) AND (GETDATE() BETWEEN M.ReleaseDate AND M.EndDate) AND M.CmsID in (SELECT CmsID FROM Relation) ")

            If txt_Key.Text.Replace("'", "").Trim <> "" Then .AppendFormat(" AND (M.Title like N'%{0}%' or M.Abstract like N'%{0}%') ", txt_Key.Text.Trim.Replace("'", "''").Replace("*", "%"))

            If arrTypes.Where(Function(x) cms_types.Contains(x.Replace("'", ""))).Count > 0 Then
                .AppendFormat(" And D.DisplayName in (" + String.Join(",", arrTypes.ToArray()) + ") ")
            End If
            If ddlPeriod.SelectedValue = "0" Then
                .AppendFormat(" and M.ModifyDate between dateadd(year,-2,getdate()) and getdate() ")
            End If
        End With

        Dim sqlfrom As String = sb.ToString
        Dim sql As String = sqlfrom

        If iclbbu > 0 Then
            Dim buList As New List(Of String)
            '2016-02-02 Erica: 若有勾選Corporate這個BU, 資料不限RBU都可抓出
            If pickCorp Then buList.Add("'Corp News'")

            For Each item As ListItem In cblBU.Items
                If item.Selected AndAlso item.Value <> "CORP" Then
                    buList.Add("'" + item.Value + "'")
                End If
            Next

            If buList.Count > 0 Then sql += " AND DR.DisplayName in (" + String.Join(",", buList.ToArray()) + ") "
        End If

        'If customer, filter by Org
        If Not Util.IsInternalUser(User.Identity.Name) Then
            Dim domainList As List(Of String) = GetCMSDomainByOrg()
            sql += " INTERSECT "
            sql += sqlfrom + " AND DR.DisplayName in ('" + String.Join("','", domainList.ToArray()) + "') "
        End If

        sql += " order by LAST_UPD desc "

        Return dbUtil.dbGetDataTable("CMS", sql)
    End Function

    Function GetSql() As String
        Dim getLit As Boolean = False, getTech As Boolean = False
        ...
        If Not getLit AndAlso Not getTech Then
            With arrSql
                .Add(GetLitSql(BUList)) : .Add(GetTechSql(BUList)) : .Add(GetSupportDownloadSql(BUList))
            End With
        End If
        ...
        With sb
            If txt_Key.Text.Trim.Replace("*", "") <> "" Then
                .AppendFormat("select distinct top 100 t.LIT_ID, t.LIT_NAME, t.LAST_UPD, t.DESC_TEXT, t.LIT_TYPE, t.FILE_EXT, cast(t.FILE_SIZE as int) as FILE_SIZE, t.FTP_URL, t.RECORD_IMG, t.RECORD_ID, t.SR_TYPE, t.HYPER_LINK ")
                ...
                If arrTypes.Count > 0 Then
                    .AppendFormat(" and t1.LIT_TYPE in ({0}) and t1.LIT_ID not in (select z.Thumbnail_ID from PIS.dbo.LITERATURE_EXTEND z (nolock) where z.Thumbnail_ID is not null or z.Thumbnail_ID !='') ", String.Join(",", arrTypes.ToArray()))
                End If
                ...
                If CInt(dbUtil.dbExecuteScalar("MY", String.Format("select COUNT(FLASHLEADS_RBU) from CurationPool.dbo.LEADSFLASHRBU_SIEBELRBU (nolock) where SIEBEL_RBU in (select RBU from SIEBEL_ACCOUNT where ERP_ID ='{0}') and FLASHLEADS_RBU in ('AEU','ANA')", Session("COMPANY_ID")))) > 0 Then
            ...
            Else
                .AppendFormat("select distinct top 100 t.LIT_ID, t.LIT_NAME, t.LAST_UPD, t.DESC_TEXT, t.LIT_TYPE, t.FILE_EXT, t.FILE_SIZE, t.FTP_URL, t.RECORD_IMG, t.RECORD_ID, t.SR_TYPE, t.HYPER_LINK ")
    ...
    Public Function GetLitSql(ByVal BUList As List(Of String)) As String
        ...
        With sb
            ...
            .AppendFormat(" FROM (select a.* from PIS.dbo.v_LITERATURE a (nolock) left join PIS.dbo.Model_lit b (nolock) on a.LITERATURE_ID=b.literature_id) a LEFT JOIN SIEBEL_LITERATURE_DETAIL b (nolock) ON a.LITERATURE_ID = b.LIT_ID ")
            .AppendFormat(" LEFT JOIN PIS.dbo.LITERATURE_EXTEND c (nolock) ON a.LITERATURE_ID = c.LIT_ID ")
            If BUList IsNot Nothing AndAlso BUList.Count > 0 Then
                .AppendFormat(" LEFT JOIN PIS.dbo.Model_lit d (nolock) on a.LITERATURE_ID=d.literature_id ")
                .AppendFormat(" LEFT JOIN PIS.dbo.MODELCATEGORY_INTERESTEDPRODUCT_MAPPING e (nolock) on d.model_name=e.ITEM_DISPLAYNAME ")
                .AppendFormat(" LEFT JOIN CurationPool.dbo.LEADSFLASH_PRODUCTCATEGORY_INTERESTEDPRODUCT f (nolock) on e.INTERESTED_PRODUCT_DISPLAY_NAME=f.Interested_Product ")
    ...
    Public Function GetTechSql(ByVal BUList As List(Of String)) As String
        ...
        With sb
            ...
            .AppendFormat(" FROM SIEBEL_SUPPORT AS a (nolock) LEFT JOIN SAP_PRODUCT b (nolock) ON a.PRODUCTS = b.MODEL_NO ")
            If BUList IsNot Nothing AndAlso BUList.Count > 0 Then
                .AppendFormat(" LEFT JOIN PIS.dbo.MODELCATEGORY_INTERESTEDPRODUCT_MAPPING e (nolock) on b.MODEL_NO=e.ITEM_DISPLAYNAME ")
                .AppendFormat(" LEFT JOIN CurationPool.dbo.LEADSFLASH_PRODUCTCATEGORY_INTERESTEDPRODUCT f (nolock) on e.INTERESTED_PRODUCT_DISPLAY_NAME=f.Interested_Product ")
                .AppendFormat(" where f.Product_Group in ({0}) ", String.Join(",", BUList.ToArray()))
            End If
        End With
    ...
    Public Function GetSupportDownloadSql(ByVal BUList As List(Of String)) As String
        ...
        With sb
            ...
            .AppendFormat(" FROM SUPPORT_DOWNLOAD a (nolock) ")
            If BUList IsNot Nothing AndAlso BUList.Count > 0 Then
                .AppendFormat(" left join SIEBEL_SR_PRODUCT c (nolock) on a.DOC_ID=c.SR_ID ")
                .AppendFormat(" left join PIS.dbo.model_product d (nolock) on c.PART_NO=d.part_no or c.PART_NO=d.model_name ")
                .AppendFormat(" LEFT JOIN PIS.dbo.MODELCATEGORY_INTERESTEDPRODUCT_MAPPING e (nolock) on d.model_name=e.ITEM_DISPLAYNAME ")
                .AppendFormat(" LEFT JOIN CurationPool.dbo.LEADSFLASH_PRODUCTCATEGORY_INTERESTEDPRODUCT f (nolock) on e.INTERESTED_PRODUCT_DISPLAY_NAME=f.Interested_Product ")
                .AppendFormat(" where f.Product_Group in ({0}) ", String.Join(",", BUList.ToArray()))
            End If
    ...
    Function GetMergeData() As DataTable
        Dim dt As DataTable = dbUtil.dbGetDataTable("MY", GetSql())
        dt.Merge(GetCMSData(), True, MissingSchemaAction.Ignore)
        Return dt
    End Function
    ...
    Protected Sub gv1_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs)
        ...
        gv1.DataSource = SortDataTable(GetMergeData(), False)
        gv1.DataBind()
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Else
                ...
                Else
                    ...
                    ElseIf lit_type = "product roadmaps" Then
                        If DataBinder.Eval(e.Row.DataItem, "HYPER_LINK").ToString.trim <> "" Then
                            e.Row.Cells(6).Text = "<a href='" + DataBinder.Eval(e.Row.DataItem, "HYPER_LINK").ToString + "' target='_blank'>Download</a>"
                        End If
            ...
            Dim DTime As Date
            If Date.TryParse(e.Row.Cells(7).Text, DTime) Then
    ...
    Public Shared Function GetNewsContent(ByVal recid As String, ByVal Type As String) As String
        Try
            Dim obj As Object = dbUtil.dbExecuteScalar("CMS", "SELECT top 1 isnull([Content],'') AS CMS_CONTENT FROM Detail WHERE CmsID='" + recid + "'")
            If obj IsNot Nothing Then Return obj.ToString()
            Return ""
        Catch ex As Exception
            MailUtil.SendEmail("MyAdvantech@advantech.com", "MyAdvantech@advantech.com", "Error GetWWWNews", "recid:" + recid + "<br/>" + ex.ToString, False, "", "")
        End Try
                                                    ...
                                                    <tr valign="top" style="width:100%;">
                                                        <td valign="top" align="left" style="width:100%;">                                                            
                                                            <asp:CheckBoxList runat="server" ID="cblBU" RepeatColumns="2" 
                                                                ...
                                                                <asp:ListItem Text="(BU)Applied Computing Platforms" Value="(BU)Applied Computing Platforms" />
                                                                <asp:ListItem Text="(BU)Certified Peripherals" Value="(BU)Certified Peripherals" />
                                                                <asp:ListItem Text="(BU)Intelligent Healthcare" Value="(BU)Intelligent Healthcare" />
                                                                <asp:ListItem Text="(BU)Intelligent Logistics" Value="(BU)Intelligent Logistics" />
                                                                <asp:ListItem Text="(BU)Embedded Computing" Value="(BU)Embedded Computing" />
                                                                ...
                                                                <asp:ListItem Text="(BU)Networks & Communications" Value="(BU)Networks & Communications" />
                                                                <asp:ListItem Text="(BU)Intelligent Retail" Value="(BU)Intelligent Retail" />
            ...
            <td>                
                <asp:Panel runat="server" ID="PanelGv">
                    <asp:GridView runat="server" ID="gv1" Width="100%" AutoGenerateColumns="False" HeaderStyle-HorizontalAlign="Center"
                        AllowPaging="True" AllowSorting="True" PageSize="200" PagerSettings-Position="TopAndBottom" 
