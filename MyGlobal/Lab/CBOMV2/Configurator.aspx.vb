...
Partial Class Lab_ConfiguratorJQ
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            If Request("BTOItem") IsNot Nothing Then
                ...
                'Dim cmdCheckBTOItemValid As New SqlClient.SqlCommand( _
                '    "select count(category_id) as c from cbom_catalog_category where category_id=@RootId and org=@ORG and parent_category_id='Root' ", _
                '    New SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("MYLOCAL").ConnectionString))

                Dim _sql As New StringBuilder
                _sql.AppendLine(" SELECT count(category_id) as c ")
                _sql.AppendLine(" FROM CBOM_CATALOG_CATEGORY_V2 WHERE ID = @RootId ")
                Dim cmdCheckBTOItemValid As New SqlClient.SqlCommand(_sql.ToString,
                                                                     New SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("CBOMV2").ConnectionString))

                With cmdCheckBTOItemValid.Parameters
                    .AddWithValue("RootId", strBTOItem)
                End With
    ...
    <Services.WebMethod()> _
    <Web.Script.Services.ScriptMethod()> _
    Public Shared Function GetCBOM(ByVal ParentCategoryId As String, ByVal ConfigQty As Integer) As String
        'Return ParentCategoryId
        Try
            Dim cBomDal As New MyCBOMDAL
            Dim dtBom As CBOMDS.CBOM_CATALOG_CATEGORYDataTable = cBomDal.GetCBOMV3(ParentCategoryId, HttpContext.Current.Session("RBU"), HttpContext.Current.Session("org_id"))
            Dim lsCBom As New List(Of CBomV2)
            For Each rBom As CBOMDS.CBOM_CATALOG_CATEGORYRow In dtBom.Rows
                Dim bom1 As New CBomV2
                bom1.ChildCategories = New List(Of CBomV2)
                With bom1
                    .ID = rBom.PARENT_CATEGORY_ID
                ...
                If bom1.CategoryType = 1 Or bom1.CategoryType = 4 Then
                    Dim dtComps As CBOMDS.CBOM_CATALOG_CATEGORYDataTable = cBomDal.GetCBOMV3(bom1.ID, HttpContext.Current.Session("RBU"), HttpContext.Current.Session("org_id"))
                    For Each rComp As CBOMDS.CBOM_CATALOG_CATEGORYRow In dtComps.Rows
                        Dim compBom As New CBomV2
                        With compBom
                            .ID = rComp.PARENT_CATEGORY_ID
