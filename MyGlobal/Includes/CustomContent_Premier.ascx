<%@ Control Language="VB" ClassName="CustomContent_Premier" %>
...
<script runat="server">
    ...
    Function GetMyEDM() As DataTable
        ...
        If Session("user_id") Is Nothing Then
            strSql = String.Format("select top 4 b.row_id, b.email_subject from CAMPAIGN_MASTER b where CAMPAIGN_NAME Like N'%eStore%' and ACTUAL_SEND_DATE is not null and CLICK_CUST>100 and b.IS_PUBLIC=1 order by CREATED_DATE desc")
        Else
    ...
    Function GetCustContent(ByVal UseBaa As Boolean) As DataTable
        ...
        Dim dicCMSSelect As Dictionary(Of String, DataTable) = HttpContext.Current.Cache("CustContent")
        If dicCMSSelect Is Nothing Then
            dicCMSSelect = New Dictionary(Of String, DataTable)
            HttpContext.Current.Cache.Add("CustContent", dicCMSSelect, Nothing, Now.AddHours(12), Cache.NoSlidingExpiration, CacheItemPriority.Default, Nothing)
        End If
        ...
        If Not dicCMSSelect.ContainsKey(strSql) Then
            ...
            dicCMSSelect.Add(strSql, dt)
        End If
        If dicCMSSelect.ContainsKey(strSql) Then
            Return dicCMSSelect.Item(strSql)
        Else
            Return dbUtil.dbGetDataTable("MY", strSql)
        End If
    End Function
    ...
    Protected Sub gv1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs)
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim url As String = ""
            If Session("user_id") Is Nothing Then
                url = "http://resources.advantech.com/Resources/Details.aspx?rid=" + gvVideo.DataKeys(e.Row.RowIndex).Values("RECORD_ID").ToString
            ...
            Else
                url = "http://resources.advantech.com.tw/sso/autologin.aspx?tempid=" + Session("TempId") + "&id=" + Session("user_id") + "&pass=MY&callbackupurl=http://resources.advantech.com/Resources/Details.aspx?rid=" + gvVideo.DataKeys(e.Row.RowIndex).Values("RECORD_ID").ToString
    ...
    Protected Sub TimerContent_Tick(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            Dim rTypes() As String = {"Video"}
            Dim rTables() As DataTable = {vidDt, newsDt, wpaperDt, csDt}
            Dim gvResources() As GridView = {gvVideo}
