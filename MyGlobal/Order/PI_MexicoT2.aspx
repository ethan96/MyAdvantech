...
<script runat="server">
    ...
    Sub initInterface()
        ...
        If dtMaster.Rows.Count > 0 And dtDetail.Rows.Count > 0 Then
            '\ Ming add for MexicoT2Customer 2013-08-26''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            Dim SoldToId As String = dtMaster.Rows(0).Item("SOLDTO_ID"), ShipToId As String = SoldToId, ParentCompany As String = String.Empty
            If Util.IsMexicoT2Customer(SoldToId, ParentCompany) Then
                SoldToId = ParentCompany
                ShipToId = ParentCompany
            End If
            ...
            For Each dr As DataRow In dtDetail.Rows
                ProdInDt.AddProductInRow(dr.Item("PART_NO"), Integer.Parse(dr.Item("Qty")), "")
            Next
            Dim retFlg As Boolean = WS.GetPrice(SoldToId, ShipToId, Session("org_id").ToString.Trim, "", "", ProdInDt, ProdOutDt, strErrMsg)
            If retFlg Then
                For Each row In dtDetail.Rows
                    Dim _row = ProdOutDt.Select(String.Format("PART_NO='{0}'", row("Part_no"))).FirstOrDefault()
                    Dim _isphaseout As Boolean = False
                    If _row IsNot Nothing Then
                        row("Unit_price") = _row.Item("UNIT_PRICE")
                    End If
                Next
            End If
            '/ end''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    Protected Sub gv1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs)
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If myOrderDetail.isBtoParentItem(Request("NO"), Me.gv1.DataKeys(e.Row.RowIndex).Value) Then
                e.Row.Cells(9).Text = Session("company_currency_sign") & FormatNumber(myOrderDetail.getTotalPrice(Request("NO")), 2)
                e.Row.Cells(10).Text = Session("company_currency_sign") & FormatNumber(myOrderDetail.getTotalAmount(Request("NO")), 2)
            End If
        End If
...
<%--        <table width="100%">
    ...
    <div id="divDetailInfo" class="mytable">
        ...
        <table width="100%">
                        ...
                        <Columns>
                          ...
                          <%--  <asp:TemplateField HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center">
                            ...
                            <asp:TemplateField HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center">
                                ...
                                <ItemTemplate>
                                     <asp:Label runat="server" Text='<%# Session("company_currency_sign")%>' ID="lbUnitPriceSign"></asp:Label> <%# FormatNumber(Eval("Unit_price"), 2)%>
                                </ItemTemplate>
                            </asp:TemplateField>
                            <asp:TemplateField HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center">
                                <HeaderTemplate>
                                ...
                                <ItemTemplate>
                                     <asp:Label runat="server" Text='<%# Session("company_currency_sign")%>' ID="lbSubTotalSign"></asp:Label> <%# FormatNumber(Eval("Unit_price") * Eval("Qty"), 2)%>
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                    </asp:GridView>
            ...
            <tr><td align="right" runat="server" id="trFreight" visible="false"> Freight：<%= HttpContext.Current.Session("company_currency_sign")%><asp:Label runat="server" ID="lbFt"></asp:Label></td></tr>
