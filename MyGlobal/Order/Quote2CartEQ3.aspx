...
<script runat="server">
    ...
    Protected Sub Page_PreRender(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim IsAllowedReduceQty As String = Request("ChangeQtyIsAllowed")
        ...
        If MyQuoteMaster IsNot Nothing AndAlso MyQuoteDetail IsNot Nothing Then
            ...
            If MyQuoteDetail.Count > 0 Then
                ...
                'Ryan 20180502 For tax rate fix
                If MyQuoteMaster.tax = 0.17 AndAlso MyQuoteMaster.quoteNo.StartsWith("ACNQ") Then
                    Dim temptaxrate As Decimal = 1 + ConfigurationManager.AppSettings("ACNTaxRate")
                    For Each qd In MyQuoteDetail
                        qd.listPrice = (qd.listPrice * 1.17 / temptaxrate)
                        qd.newUnitPrice = (qd.newUnitPrice * 1.17 / temptaxrate)
                    Next
                End If
                ...
                For Each _QuoteDetailRow As QuotationDetail In MyQuoteDetail
                    ...
                    Else
                        If _QuoteDetailRow.ItemType = -1 Then
                            currentItemType = CartItemType.BtosParent
                            _ISautoaddEX = False
                        ElseIf _QuoteDetailRow.ItemType = 1 Then
                            currentItemType = CartItemType.BtosPart
                            _ISautoaddEX = False
                        Else
                            currentItemType = CartItemType.Part
                    ...
                    If currentItemType = CartItemType.BtosParent Then
                        ...
                        _row.Item("Itp") = 0
                    Else
                        If Session("org_id") IsNot Nothing AndAlso Session("org_id").ToString.ToUpper.StartsWith("CN") Then
                            Dim taxRate As Decimal = 1 + ConfigurationManager.AppSettings("ACNTaxRate")
                            If _QuoteDetailRow.listPrice.HasValue = True Then
                                _row.Item("List_Price") = Math.Round(_QuoteDetailRow.listPrice.Value * taxRate, 2)
                            End If
                            If _QuoteDetailRow.unitPrice.HasValue = True Then
                                _row.Item("Unit_Price") = Math.Round(_QuoteDetailRow.newUnitPrice.Value * taxRate, 2)
                            End If
                        Else
                ...
                'Ryan 20171226 Set session for EQV3 qty modifing in CartListV3 
                If Not String.IsNullOrEmpty(IsAllowedReduceQty) AndAlso IsAllowedReduceQty.ToUpper.Equals("TRUE") Then
                    Session("Quote3AllowedReduceQty") = True
                Else
                    Session("Quote3AllowedReduceQty") = False
                End If
                ...
                Else
                    Response.Redirect("~/Order/Cart_listV3.aspx")
                End If
            End If
    ...
    Sub Access(ByVal User As String, ByVal Company As String, ByVal ORG As String)
        ...
        If MYSAPBIZ.is_Valid_Company_Id(Company) Then
            ...
            'Ryan 20171106 Add for Change Company with ORG due to some companies have multi org.
            If Not String.IsNullOrEmpty(ORG) Then
                Dim au As AuthUtil = New AuthUtil
                au.ChangeCompanyId(Company, ORG)
            End If
